...
def get_true_link(text, c):
    ...
    try:
        ...
        img_lst = list(imgs)
        save_img(
            json.loads(img_lst[random.randrange(0, len(img_lst))].get("data-bem"))['serp-item']['preview'][0]['url'], c)
    except(Exception):
        save_img(NOTFOUNDIMAGE, c)
...
async def on_message(message):
    ...
    if message.content.startswith('/help '):
        ...
        DebugMessage = await message.channel.send(text + "\n")
        sentenses = text.split("\n")
        ...
        for i in range(len(x1)):
            ans += x1[i] + "\n" + x3[i].replace(" Ди ", " Зэ ").replace(" ди ", " зэ ").replace("тф", "ф").replace("Тф",
                                                                                                                   "Ф").replace(
                " Тын ", " Финг ").replace(" тын ", " финг ") + "\n" + x2[i] + "\n" + "\n"
        await DebugMessage.delete()
        await message.channel.send(ans)

    if message.content.startswith('/tti '):
        text = str(message.content)[5:]
